            ...
            <div className="flex-[2] min-w-0 order-2 lg:order-1 flex flex-col h-screen">
                ...
                >
                    <div className="bg-slate-900/60 backdrop-blur-3xl border-r border-white/10 flex-1 flex flex-col p-0 text-center relative overflow-hidden transition-all duration-1000 h-full">
                        ...
                        <div className="absolute top-10 left-10 z-50 flex items-center gap-6 animate-bounce-slow">
                            <div className="w-16 h-16 lg:w-20 lg:h-20 bg-white/5 rounded-3xl flex items-center justify-center border border-white/10 shadow-2xl backdrop-blur-3xl">
                                {showWait ? (
                                    <Clock size={32} className="text-amber-500/80 lg:w-10 lg:h-10" />
                                ) : (
                                    <Stethoscope size={32} className="text-emerald-500/80 lg:w-10 lg:h-10 animate-pulse" />
                                )}
                            </div>
                            <div className="flex flex-col items-start gap-1">
                                <div className="flex items-center gap-3">
                                    <div className={`w-3 h-3 rounded-full animate-pulse shadow-[0_0_15px_rgba(16,185,129,1)] ${showWait ? 'bg-amber-500' : 'bg-emerald-500'}`}></div>
                                    <span className="text-xs lg:text-sm font-black text-emerald-400 uppercase tracking-[0.6em]">
                                        {showWait ? 'Wait' : 'Now Consulting'}
                                    </span>
                                </div>
                                <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em]">HealthCor Node Active</p>
                            </div>
                        </div>
                                ...
                                {showWait ? (
                                    ...
                                    >
                                        <div className="w-full px-4 overflow-hidden">
                                            <ScrollingName
                                                name="PLEASE WAIT"
                                                className="text-8xl sm:text-[14rem] md:text-[18rem] lg:text-[22rem] xl:text-[28rem] font-black text-white/5 tracking-tighter leading-none italic uppercase"
                                            />
                                        </div>
                                    </motion.div>
                                ) : (
                                    ...
                                    >
                                        <div className="w-full px-4 lg:px-20">
                                            <ScrollingName
                                                ...
                                                className="text-[10rem] sm:text-[14rem] lg:text-[22rem] xl:text-[28rem] font-black text-white tracking-tighter leading-none uppercase drop-shadow-[0_60px_100px_rgba(0,0,0,0.8)]"
                                            />
                                        </div>
                    ...
                    <div className="bg-black border-l border-white/5 p-10 lg:p-16 shadow-2xl flex flex-col h-full relative overflow-hidden group">
                        ...
                        <div className="flex items-center justify-between mb-8 relative z-20">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="text-3xl lg:text-4xl font-black tracking-tighter uppercase mb-1 text-white">Queue</h2>
                                    <p className="text-emerald-400 font-black text-[10px] uppercase tracking-[0.4em]">{arrivedAppointments.length} Active</p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 relative z-20 min-h-0">
                            ...
                            <div className="h-full overflow-y-auto overflow-x-hidden space-y-6 pr-4 py-20 custom-scrollbar text-left relative scroll-smooth">
                                <AnimatePresence mode="popLayout" initial={false}>
                                    {arrivedAppointments.length === 0 ? (
                                        ...
                                        >
                                            ...
                                            <p className="text-sm font-black uppercase tracking-[0.4em] text-center px-12 text-white">PLEASE WAIT</p>
                                        </motion.div>
                                    ) : (
                                        arrivedAppointments.map((appt, idx) => (
                                            <motion.div
                                                ...
                                                className={`flex items-center gap-8 p-8 lg:p-10 rounded-[2.5rem] border transition-all duration-700 ${idx === 0 ? 'bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-900 border-white/20 shadow-[0_30px_60px_-15px_rgba(5,150,105,0.6)] scale-[1.03] z-40' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}
                                            >
                                                <div className={`w-14 h-14 lg:w-18 lg:h-18 rounded-[1.2rem] lg:rounded-[1.5rem] flex-shrink-0 flex items-center justify-center font-black text-2xl lg:text-3xl shadow-inner ${idx === 0 ? 'bg-white text-emerald-800' : 'bg-white/5 text-emerald-500'}`}>
                                                    {idx + 1}
                                                </div>
                                                <div className="flex-1 overflow-hidden">
                                                    <ScrollingName
                                                        name={appt.patientName}
                                                        className={`text-2xl lg:text-3xl xl:text-4xl font-black uppercase tracking-tighter ${idx === 0 ? 'text-white' : 'text-slate-300'}`}
                                                    />
                                                    <div className={`h-1.5 w-12 rounded-full mt-3 ${idx === 0 ? 'bg-white/40' : 'bg-emerald-500/20'}`}></div>
                                                </div>
                                                {appt.emergency && (
                                                    <div className="flex-shrink-0 bg-red-500/20 p-3 rounded-full">
                                                        <Zap className="w-6 h-6 lg:w-8 lg:h-8 text-red-500 animate-pulse" />
                                                    </div>
                                                )}
                                            </motion.div>
                                        ))
                        ...
                        <div className="mt-8 pt-6 border-t border-white/10 text-center relative z-20 opacity-40">
                            <p className="text-[11px] font-black text-slate-300 uppercase tracking-[0.4em] animate-pulse">Kindly be seated until your name is called</p>
                        </div>
                    </div>
                </motion.div>
            </div>
