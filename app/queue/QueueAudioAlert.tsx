...
import { useClinicSound } from '@/components/providers/ClinicSoundProvider';
...
export default function QueueAudioAlert({ currentPatient }: { currentPatient: string }) {
              ...
              setTimeout(() => {
                if ('speechSynthesis' in window) {
                  ...
                  const msg = new SpeechSynthesisUtterance(
                    `${currentPatient}, please proceed to the consultation room.`
                  );
                  msg.rate = 0.9;
                  msg.pitch = 1.0;
                  msg.volume = 1.0;
  ...
  // Handle auto-close safely, compatible with React 18 strict mode
  useEffect(() => {
    if (showOverlay) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <Activity
          size={40}
          className="absolute bottom-8 left-8 text-indigo-500/10 animate-pulse delay-700"
        />
        ...
        <div className="w-full flex flex-col items-center gap-8">
          ...
          <div className="space-y-2 w-full px-4 overflow-hidden">
            <p className="text-slate-500 text-lg md:text-2xl font-black uppercase tracking-[0.2em] opacity-80">
              Calling Now
            </p>
          ...
          <div className="bg-white/5 border border-white/10 px-10 py-5 rounded-[2.5rem] shadow-xl animate-slide-in-up">
            <div className="flex items-center gap-5 text-emerald-400">
              ...
              <span className="text-xl md:text-4xl font-black uppercase tracking-widest leading-none">
                ROOM 01
              </span>
            </div>
          </div>
