...
import { ShieldCheck, Users, Trash2, LayoutDashboard, Database, LogOut, Activity, Zap } from 'lucide-react';
            ...
            <div className="w-full min-h-screen flex items-center justify-center p-6 bg-transparent font-outfit">
                <AnimatedWrapper tilt>
                    <div className="glass-vip-polished rounded-[3.5rem] p-16 max-w-md w-full text-center border-beam border border-red-500/20 shadow-2xl">
                        ...
                        <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-4 tracking-tighter uppercase">Access Denied</h2>
    ...
    return (
        <div className="space-y-12 animate-fade-in pb-32 font-outfit max-w-[1600px] mx-auto px-6 lg:px-0 relative">
            ...
            <AnimatedWrapper direction="down">
                <div className="glass-vip-polished border-beam rounded-[3.5rem] p-10 md:p-14 border border-white/10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.2)] flex flex-col md:flex-row justify-between items-start md:items-center gap-10 overflow-hidden relative group">
                    <div className="flex items-center gap-10 relative z-10 text-left">
                        <div className="w-24 h-24 bg-slate-950 dark:bg-indigo-600 rounded-[2rem] flex items-center justify-center text-white shadow-2xl transition-all group-hover:rotate-[-6deg] duration-700 border border-white/10">
                        ...
                        <div>
                            <div className="inline-flex items-center gap-3 px-4 py-1.5 bg-slate-100 dark:bg-white/10 rounded-full mb-3 shadow-inner">
                                ...
                                <span className="text-[10px] font-black text-slate-500 dark:text-indigo-400 uppercase tracking-[0.4em]">Root Access Node</span>
                            </div>
                            <h1 className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">Admin Control</h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 w-full md:w-auto relative z-10">
                        <div className="bg-white/50 dark:bg-black/40 border border-slate-200 dark:border-white/10 px-8 py-5 rounded-[2rem] flex items-center gap-6 shadow-xl backdrop-blur-3xl min-w-[200px]">
                            <div className="text-left">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] leading-none mb-2">Network Layer</p>
                                <p className="text-xs font-black text-emerald-500 uppercase tracking-widest leading-none">Healthy | AES-256</p>
                            </div>
                    ...
                    <AnimatedWrapper direction="right">
                        <div className="glass-vip-polished rounded-[4rem] p-10 md:p-14 border border-white/10 shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="flex justify-between items-center mb-16 px-4">
                                ...
                                <span className="bg-slate-950 dark:bg-black text-white px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] shadow-xl border border-white/5">{allAppointments.length} Records</span>
                            </div>

                            <div className="space-y-6 max-h-[1000px] overflow-y-auto pr-4 custom-scrollbar">
                                {allAppointments.map((appt) => (
                                    <div key={appt.id} className="p-10 bg-white/40 dark:bg-white/[0.03] backdrop-blur-3xl rounded-[3rem] border border-slate-200/50 dark:border-white/5 group transition-all duration-700 hover:bg-white dark:hover:bg-black hover:border-emerald-500/30 hover:shadow-2xl hover:scale-[1.01]">
                                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-10 pb-10 border-b border-slate-100 dark:border-white/5">
                                            <div className="flex items-center gap-8">
                                                <div className="w-16 h-16 bg-slate-900 dark:bg-black border border-white/10 rounded-[1.5rem] flex items-center justify-center font-black text-2xl text-slate-400 group-hover:text-emerald-500 transition-all shadow-2xl">
                                                    {appt.patientName[0]}
                                                </div>
                                                <div>
                                                    <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none group-hover:text-emerald-500 transition-colors">{appt.patientName}</h3>
                                                    <div className="flex flex-wrap items-center gap-5 mt-3 text-slate-400 font-black uppercase text-[10px] tracking-[0.3em] opacity-60">
                                                        ...
                                                        {appt.emergency && <span className="text-red-500 animate-pulse">Priority Override</span>}
                                                    </div>
                                                </div>
                                            </div>
                                            <span className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] border shadow-sm ${appt.status === 'completed' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-slate-100 dark:bg-white/5 text-slate-500 border-slate-200 dark:border-white/10'}`}>
                                                {appt.status}
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex flex-col gap-2">
                                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">Registry Timestamp</span>
                                                <span className="text-sm font-black text-slate-600 dark:text-slate-400 opacity-80 uppercase tracking-widest">{appt.date || 'Pending'} | {appt.time || '--:--'}</span>
                                            </div>
                                            <form action={async () => {
                                                'use server';
                        ...
                        <div className="glass-vip-polished rounded-[4rem] p-10 md:p-14 border border-white/10 shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="space-y-4 max-h-[850px] overflow-y-auto custom-scrollbar pr-2">
                                {allPatients.map((patient) => (
                                    <div key={patient.id} className="flex items-center justify-between p-6 bg-white/40 dark:bg-white/[0.03] backdrop-blur-3xl border border-slate-200/50 dark:border-white/5 hover:border-indigo-500/40 rounded-[2rem] transition-all duration-700 group/item hover:bg-white dark:hover:bg-black active:scale-[0.98]">
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-sm font-black text-slate-900 dark:text-white tracking-tighter uppercase group-hover/item:text-indigo-400 transition-colors">{patient.name}</span>
                                                <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1 opacity-60">ID://{patient.id}</span>
                                            </div>
                                        </div>
                                        <Activity size={12} className="text-indigo-500 opacity-0 group-hover/item:opacity-40 transition-opacity" />
                                    </div>
                                ))}
                            </div>
