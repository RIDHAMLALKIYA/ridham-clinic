...
export default function LobbyManager({ initialAppointments }: { initialAppointments: Appointment[] }) {
  ...
  return (
    <div className="glass-vip-polished rounded-[2.5rem] md:rounded-[4rem] p-5 md:p-14 border border-white/20 shadow-2xl relative overflow-hidden group">
      <div className="absolute top-0 right-10 w-40 h-40 bg-emerald-500/5 blur-[80px] rounded-full"></div>

      {/* HEADER SECTION */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 md:gap-8 mb-10 md:mb-16 px-2 md:px-4">
        <div className="flex items-center gap-4 md:gap-6">
          <div className="bg-emerald-600 p-3 md:p-4 rounded-2xl md:rounded-3xl shadow-2xl shadow-emerald-500/30">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <p className="text-[9px] md:text-[10px] font-black text-emerald-500 uppercase tracking-widest opacity-60">
        ...
        <div className="flex items-center gap-3 md:gap-4 w-full sm:w-auto">
          <button
            ...
            className={`flex-1 sm:flex-none p-4 md:p-5 rounded-xl md:rounded-2xl transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2 md:gap-3 ${isSearchActive ? 'bg-emerald-600 text-white shadow-emerald-500/20' : 'bg-white dark:bg-white/5 text-slate-400 dark:text-white/20 border border-slate-200 dark:border-white/10 hover:border-emerald-500/50'}`}
          >
            <Search size={20} className="md:size-[24px]" />
            {isSearchActive && <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">{language === 'en' ? 'Active' : 'સક્રિય'}</span>}
          </button>

          <button
            onClick={() => setFilterMode(filterMode === 'all' ? 'emergency' : 'all')}
            className={`flex-1 sm:flex-none px-4 md:px-8 py-4 md:py-5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 md:gap-3 transition-all border shadow-xl active:scale-95 ${filterMode === 'emergency'
          ...
          >
            <Zap size={16} className={filterMode === 'emergency' ? 'animate-pulse' : ''} />
            {filterMode === 'emergency' ? (language === 'en' ? 'Urgent' : 'ઇમરજન્સી') : (language === 'en' ? 'Urgent Only' : 'માત્ર ઇમરજન્સી')}
          </button>
        </div>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((appt, idx) => (
            <button
              ...
              className={`w-full p-6 md:p-8 bg-white/40 dark:bg-black/40 backdrop-blur-3xl border ${idx === 0 ? 'border-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.2)] scale-[1.01] md:scale-[1.02]' : appt.emergency ? 'border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.15)]' : 'border-slate-200/50 dark:border-white/10'} rounded-[1.8rem] md:rounded-[2.8rem] flex flex-col justify-between group/live transition-all duration-700 hover:bg-white dark:hover:bg-black hover:border-emerald-500 hover:shadow-2xl hover:scale-[1.02] md:hover:scale-[1.03] text-left h-full relative overflow-hidden disabled:opacity-50`}
            >
              {/* RANK & NEXT BADGE */}
              <div className="flex items-start justify-between w-full mb-6 relative z-10">
                <div className="flex flex-col gap-1">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 dark:bg-black text-white rounded-xl md:rounded-2xl flex items-center justify-center font-black text-slate-300 dark:text-white/10 transition-transform group-hover/live:scale-110 shadow-xl border border-white/5">
                    {idx + 1}
                  </div>
                  <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">{language === 'en' ? 'Rank' : 'ક્રમ'}</span>
                </div>

                {idx === 0 && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-amber-500 text-white px-3 md:px-4 py-1.5 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest shadow-xl flex items-center gap-1.5 md:gap-2 animate-bounce">
                    <Zap size={9} fill="currentColor" />
                    {language === 'en' ? 'Next' : 'આગળ'}
                  </div>
                )}

                <div className="flex flex-col items-end gap-1.5 md:gap-2">
                  <div
                    className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center transition-all shadow-2xl ${appt.emergency ? 'bg-red-600 text-white animate-pulse' : 'bg-emerald-600 text-white group-hover/live:rotate-12'}`}
                  >
                    <BellRing size={20} className="md:size-[24px]" />
                  </div>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="p-1.5 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all opacity-100 sm:opacity-0 group-hover/live:opacity-100"
                  >
                    <CloseIcon size={12} />
                  </button>
                  <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest ${appt.emergency ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}>
                    {appt.emergency ? 'Urgent' : (language === 'en' ? 'Priority' : 'પ્રાથમિકતા')}
                  </span>
                </div>
              </div>

              <div className="space-y-3 md:space-y-4">
                <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tighter text-xl md:text-2xl truncate group-hover/live:text-emerald-500 transition-colors">
                  {appt.patientName}
                </h3>
                <div className="flex items-center gap-2 md:gap-3">
                  <Activity size={10} className="text-emerald-500 opacity-40 md:size-[12px]" />
                  <p className="text-xs md:text-sm font-bold text-slate-500 dark:text-white/40">{appt.phone}</p>
                </div>
                <div className="flex items-center gap-2 md:gap-3 opacity-60">
                  <Clock size={10} className="text-emerald-500 md:size-[12px]" />
                  <p className="text-[9px] md:text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-emerald-500/50">
