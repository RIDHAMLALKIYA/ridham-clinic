...
export default function RequestManager({ initialRequests }: { initialRequests: RequestAppt[] }) {
    ...
    return (
        <div className="glass-vip-polished rounded-[2.5rem] md:rounded-[3.5rem] p-5 md:p-10 border border-white/20 shadow-2xl relative overflow-hidden group">
            <div className="flex items-center justify-between mb-8 md:mb-10 text-left px-2">
                <h2 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white flex items-center gap-3 md:gap-4 uppercase tracking-tighter">
            ...
            <div className="space-y-4 md:space-y-5 max-h-[600px] md:max-h-[650px] overflow-y-auto pr-2 md:pr-3 custom-scrollbar">
                {requests.length === 0 ? (
                    ...
                    requests.map((appt) => (
                        <div
                            ...
                            className={`p-6 md:p-8 bg-slate-50/50 dark:bg-white/5 rounded-[1.8rem] md:rounded-[2.5rem] border border-slate-100 dark:border-white/5 space-y-4 md:space-y-6 text-left transition-all duration-500 hover:border-emerald-500/40 hover:scale-[1.01] shadow-sm hover:shadow-xl group/card ${isPending ? 'opacity-50 pointer-events-none' : ''}`}
                        >
