...
export default function FollowupManager({ initialList }: { initialList: FollowUpAppt[] }) {
    ...
    return (
        <div className="space-y-8 md:space-y-10 mt-8 md:mt-12 bg-white/5 p-5 md:p-12 rounded-[2.5rem] md:rounded-[4rem] border border-white/10 backdrop-blur-3xl shadow-2xl">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 md:gap-6 px-2 md:px-4">
                <div>
                    <h2 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white flex items-center gap-4 md:gap-6 tracking-tighter uppercase">
                        <RotateCcw className="w-8 h-8 md:w-10 md:h-10 text-emerald-600 animate-spin-slow" />
                        Follow-ups
                    </h2>
                    <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1 md:mt-2">
                        Recent Completed Cases
                    </p>
                </div>
                <span className="w-fit bg-emerald-500/10 text-emerald-500 px-4 md:px-6 py-2 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest border border-emerald-500/20">
                    {list.length} History
                </span>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                {list.length === 0 ? (
                    ...
                    list.map((appt) => (
                        <div
                            key={appt.id}
                            className={`p-6 md:p-10 bg-white/40 dark:bg-black/40 backdrop-blur-3xl border border-slate-200/50 dark:border-white/10 rounded-[2.2rem] md:rounded-[3.5rem] space-y-6 md:space-y-8 transition-all duration-700 hover:scale-[1.01] hover:shadow-2xl hover:border-emerald-500/30 group ${isPending ? 'opacity-50 pointer-events-none' : ''}`}
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4 md:gap-6">
                                    <div className="w-12 h-12 md:w-16 md:h-16 bg-emerald-600/10 rounded-xl md:rounded-[1.5rem] flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-all duration-500 border border-emerald-500/20 shadow-inner">
                                        <UserCheck className="size-[20px] md:size-[28px]" />
                                    </div>
                                    <div>
                                        <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tighter text-xl md:text-2xl group-hover:translate-x-1 transition-transform">
                                            {appt.patientName}
                                        </h3>
                                        <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5 md:mt-1 opacity-60">Verified Patient Record</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => handleDismiss(appt.id)}
                                    title="Dismiss from list"
                                    className="p-3 md:p-4 bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-red-500 hover:bg-red-500/10 rounded-xl md:rounded-2xl transition-all active:scale-90 border border-transparent hover:border-red-500/20 group/close"
                                >
                                    <X className="size-[16px] md:size-[20px] group-hover:rotate-90 transition-transform duration-500" />
                                </button>
                            </div>
